...
fig11.update_layout(
    **base_layout(height=560, margin=dict(l=20,r=20,t=70,b=40)),
    title=title("Cluster Transitions: 1995 → 2019",
                f"{len(trans)} countries tracked · flows show cluster movements"),
)
...
m3a = m3a_raw.get_robustcov_results(cov_type="cluster", groups=r3a["Country Code"])
m3a.params = pd.Series(m3a.params, index=X3a.columns)
m3a.bse    = pd.Series(m3a.bse,    index=X3a.columns)
...
m3b = m3b_raw.get_robustcov_results(cov_type="cluster", groups=r3b["Country Code"])
m3b.params = pd.Series(m3b.params, index=X3b.columns)
m3b.bse    = pd.Series(m3b.bse,    index=X3b.columns)
...
fig12.update_layout(
    **base_layout(height=560, margin=dict(l=170,r=40,t=70,b=70)),
    title=title("Coefficient Estimates: Model 3a vs 3b",
                "95% CI · Country-clustered SE (54 groups)"),
...
fig13.update_layout(
    **base_layout(height=520, margin=dict(l=70,r=40,t=70,b=60)),
    title=title("ECI vs Human Capital, by Production Value Quartile",
                "Dotted lines = OLS trendline per quartile · 54-country sample"),
...
fig14.update_layout(
    **base_layout(height=440, margin=dict(l=70,r=40,t=70,b=60)),
    title=title("ECI Distribution: 1995 vs 2019", "54-country sample"),
    barmode="overlay",
...
fig17.update_layout(
    **base_layout(height=560, margin=dict(l=160,r=40,t=70,b=60)),
    title=title("Natural Resource Rent Decomposition: Top 20 Countries",
                "% of GDP · 2019 · includes forestry rents"),
    barmode="stack",
...
for sname, slist in samples.items():
    ...
    rob_.params = pd.Series(rob_.params, index=X_.columns)
    rob_.bse    = pd.Series(rob_.bse,    index=X_.columns)
    sample_results[sname] = rob_

SAMP_COLORS = {"Original 54": "#4a6fa5", "Adj ≥3%": "#c23a3a", "Adj ≥2%": "#2e7d4a", "Adj ≥1%": "#c9a227"}
...
fig18.update_layout(
    **base_layout(height=460, margin=dict(l=170,r=40,t=70,b=60)),
    title=title("Coefficient Comparison Across Sample Definitions",
                "Model 3a · 95% CI · Clustered SE"),
    xaxis=dict(title="Coefficient", gridcolor=GRID),
...
fig19.update_layout(
    **base_layout(height=400, margin=dict(l=70,r=40,t=70,b=60)),
    title=title("R² Comparison Across Sample Definitions", "Model 3a (no lag)"),
...
fig20.update_layout(
    **base_layout(height=480, margin=dict(l=70,r=40,t=70,b=60)),
    title=title("Total vs Adjusted NR Rents (2019)",
                "Adjusted = Total minus forestry and coal rents · 45° line = no change"),
...
fig21.update_layout(
    **base_layout(height=580, margin=dict(l=165,r=40,t=70,b=60)),
    title=title("LASSO Coefficient Stability Across Samples",
                "Standardised inputs · Excluding lagged ECI"),
    barmode="group",
...
if len(r3c) > 50:
    ...
    m3c = m3c_raw.get_robustcov_results(cov_type="cluster", groups=r3c["Country Code"])
    m3c.params = pd.Series(m3c.params, index=X3c.columns)
    m3c.bse    = pd.Series(m3c.bse,    index=X3c.columns)
else:
    m3c = None
...
if len(r3d) > 50:
    ...
    m3d = m3d_raw.get_robustcov_results(cov_type="cluster", groups=r3d["Country Code"])
    m3d.params = pd.Series(m3d.params, index=X3d.columns)
    m3d.bse    = pd.Series(m3d.bse,    index=X3d.columns)
else:
    m3d = None
